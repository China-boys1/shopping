...
//	$url = $siteurl.'?mod=buy&cid='.$tool['cid'].'&tid='.$tid;
 $url='https://qjhyz-1307591433.cos-website.ap-beijing.myqcloud.com/buy.html?tid='.$tid.'&url='.$fzurl.'';
}else{
    $url='https://qjhyz-1307591433.cos-website.ap-beijing.myqcloud.com/buy.html?tid='.$tid.'&url='.$fzurl.'';
...
imagejpeg($image_goods, "file/goods_{$tid}.jpg", 100);
...
imagecopymerge($image_res,$image_goods, 40,120,0,0,560,550, 100);
imagecopymerge($image_res,$image_qrcode, 40,690,0,0,200,200, 100);
...
$fontColor = imagecolorallocate($image_res, 0, 0, 0);//字的RGB颜色
$fontBox = imagettfbbox($fontSize, 0, $font, $text);//文字水平居中实质
imagettftext($image_res, $fontSize, 0, 250, 730, $fontColor, $font, $text);

$text='￥'.$price;
$fontSize = 30;//磅值字体
$fontColor = imagecolorallocate($image_res, 225, 6, 2);//字的RGB颜色
$fontBox = imagettfbbox($fontSize, 0, $font, $text);//文字水平居中实质
imagettftext($image_res, $fontSize, 0, 250, 870, $fontColor, $font, $text);
