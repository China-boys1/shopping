...
case 'delTask':
	adminpermission('site', 2);
	$id=intval($_GET['id']);
	$sql="DELETE FROM pre_sitetask WHERE id='$id' limit 1";
	if($DB->exec($sql)!==false){
		exit('{"code":0,"msg":"删除分站任务成功！"}');
	}else
		exit('{"code":-1,"msg":"删除分站任务失败！'.$DB->error().'"}');
break;
case 'setTask': //商品上下架
	adminpermission('site', 2);
	$id=intval($_GET['id']);
	$active=intval($_GET['active']);
	$DB->exec("update pre_sitetask set active='$active' where id='{$id}'");
	exit('{"code":0,"msg":"succ"}');
break;
case 'setSite': //开启关闭站点
	adminpermission('site', 2);
