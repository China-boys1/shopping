...
if (isset($upid)) {
    	$sp=$DB->getRow("SELECT * FROM pre_tools WHERE tid='{$upid}' LIMIT 1");
    ?>
    <div class="col-sm-12 col-md-12  col-lg-12 center-block" style="margin-top:50px ">
<div class="col-sm-12 col-md-12">
<div class="block">
<div class="block-title"><h3 class="panel-title">修改商品代码</h3></div>




<div class="form-group">
<label id="username">商品名称:</label><br>
<input type="text" class="form-control" name="username" value="<?php echo $sp['name']?>" required="">
</div>
<div class="form-group">
<label id="password">tid:</label><br>
<input type="text" class="form-control" id="vid" value="<?php echo $sp['tid']?>" required="">
</div>
<div class="form-group" >
<label id="paypwd">代码:</label><br>
<input type="text" class="form-control" id="vdm" value="<?php echo htmlspecialchars($sp['daim'])?>">
</div>

<input type="submit" onclick="tij()" class="btn btn-primary btn-block" value="确定修改">
    ...
    function tij(){
        var tid=$("#vid").val();
         var dai=$("#vdm").val();
        	$.ajax({
		type : 'POST',
		url : 'ajax_shop.php?act=updaim',
		data : {tid:tid,dai:dai},
		dataType : 'json',
...
<select name="aid"><option selected>批量操作</option><option value="10">&gt;改加价模板</option><option value="11">&gt;改商品库存</option><option value="1">&gt;改为显示</option><option value="2">&gt;改为隐藏</option><option value="3">&gt;改为上架中</option><option value="4">&gt;改为已下架</option><option value="5">&gt;删除选中</option<option value="11">&gt;改商品库存</option></select><button type="button" onclick="change()">执行</button>&nbsp;&nbsp;
