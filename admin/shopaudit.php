...
<h2>用户共投稿 <b><?php echo $numrows ?></b> 个商品</h2></div>
	  ...
	  <div class="table-responsive">
        <table class="table table-striped table-bordered table-vcenter orderList">
          <thead><tr><th>商品名称</th><th>商品价格设置</th><th>所属用户zid</th><th>状态</th><th>操作</th></tr></thead>
          <tbody>
<?php
...
echo '<tr '.($sup==1?'style="background-color:#454e59;"':'').'><td><a href="javascript:show('.$res['tid'].')" style="color:#000">'.$res['name'].'</a></td>'.($res['prid']>0?'<td><span onclick="getPrice('.$res['tid'].')"><font color="blue">'.$price_class[$res['prid']].'</font>&nbsp;(成本:'.$res['price'].')</span></td>':'<td><span onclick="getPrice('.$res['tid'].')">'.$res['price'].'｜'.$res['cost'].'｜'.$res['cost2'].'</span></td>').'<td><a href="./userlist.php?my=edit&zid='.$res['pro'].'" style="color:#000">zid:'.$res['pro'].'</a></td><td>'.($res['close']==1?'
