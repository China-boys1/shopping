...
case 'editClass':
	...
	if($name==null)
		exit('{"code":-1,"msg":"分类名不能为空"}');
	if($DB->exec("update pre_class set name='$name',nr='$nr' where cid='{$cid}'")!==false)
...
case 'getBlockPay':
	...
	exit(json_encode($result));
break;
case 'setclasstemplate':
	adminpermission('shop', 2);
	$cid=intval($_POST['cid']);
	$data=trim($_POST['data']);
	if($DB->exec("update pre_class set template='$data' where cid='{$cid}'")!==false)
		exit('{"code":0,"msg":"设置成功"}');
	else
		exit('{"code":-1,"msg":"设置失败！'.$DB->error().'"}');
break;
