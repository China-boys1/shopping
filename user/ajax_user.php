...
switch($act){
	...
	$row=$DB->getRow("SELECT * FROM pre_sitetask WHERE id='$id' AND active=1 LIMIT 1");
	if(!$row)
	...
	if(!in_array($id,explode(',',$userrow['taskreceive']))){
		...
		$DB->exec("UPDATE pre_sitetask SET quantity=quantity-1 WHERE id='$id'");
		$DB->exec("UPDATE pre_sitetask SET count=count+1 WHERE id='$id'");
		$DB->exec("UPDATE pre_site SET taskreceive='".$taskreceive_n."' WHERE zid='{$userrow['zid']}'");
...
break;
case 'xiaji_up_price':
    if(!$islogin2)exit('{"code":-1,"msg":"失败"}');
      $zid=$_POST['zid'];
    $type=$_POST['type'];
   	$up=intval($_POST['up']);
    ...
    unset($islogin2);
		...
		while($row=$sql->fetch()){
			...
			$price = $price_obj->getToolCost($row['tid']);//‰∏ãÁ∫ßÊàêÊú¨‰ª∑
			$xs_price = $price_obj->getToolPrice($row['tid']);//ÈîÄÂîÆ‰ª∑
            if($type == 1){
                $a=(float)$up/100; 
...
break;
case 'auto_price':
    $type=$_POST['auto_price'];
    $num=$_POST['auto_price_num'];
    if($type=='0'){
        //关闭
   	$DB->exec("UPDATE pre_site SET inbr='0' WHERE zid='{$userrow['zid']}'");
    }elseif($type=='1'){
        //固定
    $DB->exec("UPDATE pre_site SET inbr='1',ingd='$num' WHERE zid='{$userrow['zid']}'");
    }else{
        //百分比
     $DB->exec("UPDATE pre_site SET inbr='2',inbfb='$num' WHERE zid='{$userrow['zid']}'");
