...
case 'login':
	...
	$code=daddslashes($_POST['code']);
		 ...
		 session_start();
	 if ($DB->getRow("SELECT zid FROM pre_site WHERE sj=:sj LIMIT 1", [':sj'=>$phone])) {
	
	}else{
	    	exit('{"code":-1,"msg":"手机号不存在！"}');
	}
	 if($_SESSION['code']!=$code){	exit('{"code":-1,"msg":"验证码错误"}');}
	 
	 	  $row=$DB->getRow("SELECT zid,user,pwd,sj,status FROM pre_site WHERE sj=:sj LIMIT 1", [':sj'=>$phone]);
	if($row && $phone===$row['sj']) {
		if($row['status']==0){
...
$srow=$DB->getRow("SELECT * FROM pre_site WHERE zid='$zid' LIMIT 1");
		...
		log_result('分站登录', 'User:'.$phone.' IP:'.$clientip, null, 1);
	...
	if($row && $user===$row['user'] && $pass===$row['pwd']) {
	  ...
	  $row=$DB->getRow("SELECT zid,user,pwd,sj,status FROM pre_site WHERE sj=:sj LIMIT 1", [':sj'=>$user]);
	if($row && $user===$row['sj'] && $pass===$row['pwd']) {
		if($row['status']==0){
			exit('{"code":-1,"msg":"当前账号已被封禁！"}');
		}
		$zid=$row['zid'];
...
case 'regcode':
      session_start();
      $codeyzm=$_POST['phone_verify'];
     if($codeyzm!=$_SESSION['vc_code']){$result2 = ['code'=>0, 'msg'=>'验证码错误'];exit(json_encode($result2));die;}
    if(isset($_SESSION['time'])){
    $cj=time()-$_SESSION['time'];
    $cha=10-$cj;
    if($cj<=10){$result2 = ['code'=>0, 'msg'=>$cha.'秒后再发送短信'];exit(json_encode($result2));}}
    $phone=$_POST['phone'];
    $code=rand(1321,9999);
    $statusStr = array(
"0" => "短信发送成功",
...
$user = $conf['dxzh']; //短信平台帐号
$pass = md5($conf['dxmm']); //短信平台密码
if($_POST['type']=="login"){$content="登录验证码".$code;}else{$content="注册验证码".$code;}
//$content="注册验证码".$code;//要发送的短信内容
...
$msg=$statusStr[$result];
 if(!$result){
     $_SESSION['phone']=$phone;
      $_SESSION['code']=$code;
       $_SESSION['time']=time();
        $result2 = ['code'=>1, 'msg'=>$msg];
 }else{$result2 = ['code'=>0, 'msg'=>$msg];}
	...
	if (!preg_match('/^[a-zA-Z0-9\x7f-\xff]+$/',$user)) {
	 ...
	 session_start();
	 $phone = trim(daddslashes($_SESSION['phone']));
	 if ($DB->getRow("SELECT zid FROM pre_site WHERE sj=:sj LIMIT 1", [':sj'=>$phone])) {
		exit('{"code":-1,"msg":"手机号已存在！"}');
	}
    if($_SESSION['code']!=$code){	exit('{"code":-1,"msg":"验证码错误"}');}
	...
	$sql="insert into `pre_site` (`upzid`,`power`,`domain`,`domain2`,`user`,`pwd`,`rmb`,`qq`,`sitename`,`keywords`,`description`,`anounce`,`bottom`,`modal`,`addtime`,`lasttime`,`status`,`sj`) values (:upzid,0,NULL,NULL,:user,:pwd,'0',:qq,NULL,NULL,NULL,NULL,NULL,NULL,NOW(),NOW(),'1',:sj)";
	$data = [':upzid'=>$siterow['zid']?$siterow['zid']:0, ':user'=>$user, ':pwd'=>$pwd, ':qq'=>$qq, ':sj'=>$phone];
	if($DB->exec($sql, $data)){
		$zid = $DB->lastInsertId();
