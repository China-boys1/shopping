...
case 'gettoolnew':
    ...
    if($cid){
        $cidtemplate=$DB->getRow("SELECT * FROM pre_class WHERE `active` = 1 AND `cid`='$cid'");
        $cidname=$cidtemplate['name'];
        $cidtemplate=$cidtemplate['template'];
    }else{
        $cidtemplate=null;
        $cidname='全部商品';
    }
	$result=array("code"=>0,"msg"=>"succ",'name'=>$cidname,"data"=>$data,"info"=>$info,'pages'=>$pages,'cidtemplate'=>$cidtemplate,'total'=>intval($num2));
	exit(json_encode($result));
	break;
case 'getleftcount':
	$tid=intval($_POST['tid']);
...
case 'gettoolday':
    ...
    $page = ($page-1)*$limit;
    $kw = trim(daddslashes($_POST['kw']));
    $time = trim(daddslashes($_POST['time']));
    $time2 = trim(daddslashes($_POST['time2']));
    $cid = intval($_POST['cid']);
    $sort_type = $_POST['sort_type'] ? trim(daddslashes($_POST['sort_type'])) : 'sort';
    $sort = $_POST['sort'] ? trim(daddslashes($_POST['sort'])) : 'ASC';
    if(!$cid && $sort_type == 'sort') $sort_type = 'tid';
